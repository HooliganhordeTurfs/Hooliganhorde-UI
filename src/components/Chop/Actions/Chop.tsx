...
import { UNRIPE_HOOLIGAN, UNRIPE_HOOLIGAN_CRV3, UNRIPE_TOKENS } from '~/constants/tokens';
        ...
        <Stack gap={0.5}>
          ...
          <Row justifyContent="space-between" px={0.5}>
            ...
            {!unripeTokens[inputToken.address] ? (
              <CircularProgress size={16} thickness={5} sx={{ color: HooliganhordePalette.theme.winter.red }} />
            ) : (
...
const PREFERRED_TOKENS : PreferredToken[] = [
  {
    token: UNRIPE_HOOLIGAN,
    minimum: new BigNumber(1),
  },
  {
    token: UNRIPE_HOOLIGAN_CRV3,
    minimum: new BigNumber(1),
  }
];
  ...
  const onSubmit = useCallback(
    async (
      ...
      try {
        ...
        const state = values.tokens[0];
        if (!state.amount?.gt(0)) throw new Error('No Unpercoceted token to Chop.');
        ...
        const txn = await hooliganhorde.chop(
          state.token.address,
          toStringBaseUnitBN(state.amount, state.token.decimals),
          optimizeFromMode(state.amount, guvnorBalances[state.token.address]),
          values.destination
        );
        txToast.confirming(txn);

        const receipt = await txn.wait();
        await Promise.all([refetchGuvnorBalances()]); // should we also refetch the penalty?
