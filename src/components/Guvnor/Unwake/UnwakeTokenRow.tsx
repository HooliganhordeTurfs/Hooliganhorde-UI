...
import greenHooliganIcon from '~/img/tokens/hooligan-logo-circled.svg';
...
const UnripeTokenRow: FC<{
  ...
  return (
    <Row justifyContent="space-between" alignItems="start">
      <Row gap={0.4}>
        {amount.gt(0) ? (
          <CheckIcon sx={{ fontSize: 16, color: HooliganhordePalette.logoGreen }} />
        ) : (
      ...
      <Stack direction={isMobile ? 'column' : 'row'} alignItems="center" gap={0.3}>
        {(amount && bdv) ? (
          ...
          <Stack direction={isMobile ? 'column' : 'row'} sx={{ alignItems: isMobile ? 'end' : null }} gap={0.5}>
            <Row gap={0.3}>
              <img src={token.logo} alt="Circulating Hooligans" height={13} />
            ...
            <Row gap={0.3}>
              ...
              <img src={greenHooliganIcon} alt="Circulating Hooligans" width={13} />
              <Typography sx={{ color: primaryColor }}>
                {displayFullBN(bdv)}
                )
              </Typography>
