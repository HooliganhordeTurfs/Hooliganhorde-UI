...
import useEventCache from '~/hooks/guvnor/useEventCache';
...
export type GetQueryFilters = (
  /**
   * The Guvnor account which we're querying against.
   */
  account: string,
  /**
...
/**
 * Design notes (Firm Chad)
 * ------------------------
 * 
 * Try to call a subgraph -> formulate data
 *  - Loop through data as necessary (probably fetching all data for the guvnor)
 ...
 * 
 * How to save events:
 *  1 Within each section of state (firm, field, market)
 ...
 *    
 *  2 In a top level "guvnor/events" section that shares all events
 ...
 *          out of order? Ex. I ingest all of the Firm events in order, then do all
 ...
 *        block from which to query firm events is the one at which Firm deposits first begin
 *        getting updated.
