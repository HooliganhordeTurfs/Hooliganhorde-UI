...
import { HooliganhordePrice__factory } from '~/generated';

const provider = new ethers.providers.AlchemyProvider(1, process.env.VITE_ALCHEMY_API_KEY);
const address  = HOOLIGANHORDE_PRICE_ADDRESSES[1];
const contract = HooliganhordePrice__factory.connect(address, provider);

/**
 * Return a JSON version of the Hooliganhorde price contract's
