...
const VoteForm: FC<FormikProps<VoteFormValues> & {
  ...
  ///  Quorum
  const { data: { totalHorde, hordeForQuorum, pctHordeForQuorum: quorumPct, votingPower, tag }, loading: loadingQuorum } = quorum;
  ...
  const canVote = guvnorFirm.horde.active.gt(0);
    ...
    <Form autoComplete="off">
      <Stack gap={1}>
        ...
        <Stack px={1} pb={1} gap={1.5}>
          {(votingPower && totalHorde) && (
            <StatHorizontal
              label="Voting Power"
              labelTooltip={
                <div>
                  <Typography>A snapshot of your active HORDE when voting on {tag} began.</Typography>
                </div>
              }
            >
              {displayBN(votingPower)} HORDE&nbsp;·&nbsp;{displayBN(votingPower.div(totalHorde).multipliedBy(100))}%
            </StatHorizontal>
          )}
          {(quorumPct && hordeForQuorum) && (
            <StatHorizontal
              label={(
                <>
                ...
                <Stack gap={0.5}>
                  {hordeForQuorum && (
                    <StatHorizontal label="Horde for Quorum">
                      ~{displayFullBN(hordeForQuorum, 2, 2)}
                    </StatHorizontal>
                  )}
                  <StatHorizontal label="Eligible Horde">
                    ~{displayFullBN(totalHorde || ZERO_BN, 2, 2)}
                  </StatHorizontal>
            ...
            >
              {loadingQuorum ? (
                ...
                <>
                  ~{displayFullBN(hordeForQuorum, 0)} HORDE&nbsp;·&nbsp;{(quorumPct * 100).toFixed(0)}%
                </>
              )}
            </StatHorizontal>
          )}
            ...
            <Stack gap={0.5} key={choice}>
              <Row columnGap={0.5} flexWrap="wrap" justifyContent="space-between">
                ...
                <Typography variant="body1" color="text.secondary">
                  {displayFullBN(new BigNumber(proposal.scores[index]), 0, 0)} HORDE
                  <Typography
    ...
    async (
      ...
      try {
        ...
        txToast = new TransactionToast({
          ...
          success: 'Vote successful. It may take some time for your vote to appear on the Hooliganhorde UI. Check Snapshot for the latest results.',
        });
