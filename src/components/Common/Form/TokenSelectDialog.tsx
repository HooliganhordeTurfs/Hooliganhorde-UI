...
import { HooliganhordePalette, FontSize, IconSize } from '../../App/muiTheme';
...
export type TokenBalanceMode = {
  'farm': GuvnorBalances;
  'firm-deposits': GuvnorFirm['balances'];
}
...
export type TokenSelectDialogProps<K extends keyof TokenBalanceMode> = {
   ...
   */
  balancesType?: K;
  /** The Guvnor's current balances. Displayed alongside each token.
  ...
  // balances: GuvnorFirmBalance['deposited'] | FarmerBalances | undefined;
...
const TokenSelectDialog : TokenSelectDialogC = React.memo(({
  ...
  const getBalance = useCallback((addr: string) => {
    ...
    return (_balances as TokenBalanceMode['firm-deposits'])?.[addr]?.deposited?.amount || ZERO_BN;
        ...
        <List sx={{ p: 0 }}>
          ...
          {_balances ? (
            <Typography ml={1} pt={0.5} textAlign="center" fontSize={FontSize.sm} color="gray">
              ...
              <Link href="https://docs.hooligan.money/almanac/protocol/asset-states" target="_blank" rel="noreferrer" underline="none">
                Learn more &rarr;
              </Link>
            </Typography>
          ) : null}
