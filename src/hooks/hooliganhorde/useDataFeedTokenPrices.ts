...
import { updateTokenPrices } from '~/state/hooliganhorde/tokenPrices/actions';
...
export default function useDataFeedTokenPrices() {
  const tokenPriceMap = useSelector<AppState, AppState['_hooliganhorde']['tokenPrices']>((state) => state._hooliganhorde.tokenPrices);
  ...
  const fetch = useCallback(async () => {
    ...
    console.debug('[hooliganhorde/tokenPrices/useCrvUnderlylingPrices] FETCH');
    ...
    console.debug(`[hooliganhorde/tokenPrices/useCrvUnderlyingPrices] RESULT: ${priceDataCache}`);
