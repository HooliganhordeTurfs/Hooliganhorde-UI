...
const RewardsForm: React.FC<RewardsFormProps> = ({ open, children }) => {
  ...
  const [fetchGuvnorFirm] = useFetchFarmerFirm();
  ...
  const estimateGas = useCallback(async () => {
    ...
    const _calls: ClaimCalls = {
      [ClaimRewardsAction.MOW]: {
        estimateGas: () => hooliganhorde.estimateGas.update(account),
        execute: () => hooliganhorde.update(account),
        enabled: guvnorFirm.horde.grown.gt(0),
      },
      [ClaimRewardsAction.PLANT_AND_MOW]: {
        estimateGas: () => hooliganhorde.estimateGas.plant(),
        execute: () => hooliganhorde.plant(),
        enabled: guvnorFirm.prospects.earned.gt(0),
      },
      [ClaimRewardsAction.ENROOT_AND_MOW]: {
        estimateGas: () =>
          hooliganhorde.estimateGas.farm([
            // PLANT_AND_MOW
            hooliganhorde.interface.encodeFunctionData('plant', undefined),
            // ENROOT_AND_MOW
            ...enrootData,
          ]),
        execute: () =>
      ...
      /* (
          enrootData.length > 1
            ...
            ? {
              estimateGas: () => hooliganhorde.estimateGas.farm(enrootData),
              execute:     () => hooliganhorde.farm(enrootData),
              enabled:     true,
            }
