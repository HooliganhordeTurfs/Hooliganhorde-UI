...
type GraphProps = {
  ...
  height: number;
  guvnorPlots: PlotMap<BigNumber>;
  rookieLineSize: BigNumber;
}
...
const RookieLineChart: FC<GraphProps> = withTooltip(({
  width,
  draftableIndex,
  height,
  guvnorPlots,
  ...
  const xScale = scaleLinear<number>({
    domain: [0, rookieLineSize.toNumber()],
    range: [0, width - 49]
  });
  ...
  return (
    <svg width={width} height={height}>
      <Bar
        ...
        rx={10}
      />
      {Object.keys(guvnorPlots).map((key) => (
        <Tooltip
          key={key}
          title={(
            <Stack width={150}>
              <Row gap={1} justifyContent="space-between">
                ...
                <Typography>{displayBN(new BigNumber(key).minus(draftableIndex))}</Typography>
              </Row>
              <Row gap={1} justifyContent="space-between">
                <Typography>Num Rookies:</Typography>
                <Typography>{displayBN(new BigNumber(guvnorPlots[key]))}</Typography>
              </Row>
            </Stack>
          )}
        >
          {plotMarker2(xScale(new BigNumber(key).minus(draftableIndex).toNumber()), 2)}
        </Tooltip>
      ))}
    </svg>
  );
...
const SimpleRookieLineChart: FC<{
  draftableIndex: BigNumber;
  guvnorPlots: PlotMap<BigNumber>;
  rookieLineSize: BigNumber;
}> = (props) => (
  <ParentSize debounceTime={10}>
    {({ width: visWidth, height: visHeight }) => (
      <RookieLineChart
        draftableIndex={props.draftableIndex}
        width={visWidth}
        height={50}
        guvnorPlots={props.farmerPlots}
        rookieLineSize={props.podLineSize}
      />
    )}
  </ParentSize>
);

export default SimpleRookieLineChart;
