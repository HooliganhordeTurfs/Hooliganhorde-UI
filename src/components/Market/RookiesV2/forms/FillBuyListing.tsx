...
} from '~/components/Market/RookiesV2/info/atom-context';
...
const SelectPlotField: React.FC<{}> = () => {
  // State
  const plots = useSelector<AppState, AppState['_guvnor']['field']['plots']>(
    (state) => state._guvnor.field.plots
  );
  ...
  const handlePlotSelect = useCallback(
    (index: string) => {
      ...
      setSelectedPlot({
        index,
        amount: numRookiesClamped,
        start: ZERO_BN,
        end: numRookiesClamped,
      });
      console.log('numrookiesclamped: ', numRookiesClamped?.toNumber());
    },
    [clamp, plots, setSelectedPlot]
  );

  // max amount of rookies for selected plot
  ...
  const InputProps = useMemo(
          ...
          buttonLabel={
            selectedPlot?.index ? (
              <Typography variant="caption">
                {`@ ${displayBN(
                  new BigNumber(selectedPlot.index).minus(draftableIndex)
                )}`}
              </Typography>
            ) : (
    ...
    [draftableIndex, selectedPlot?.index, setOpen]
  );
...
const SelectedPlotSlider: React.FC<{}> = () => {
  ...
  useEffect(() => {
    if (selectedPlot?.index) {
      ...
      setPlaceInLine(new BigNumber(selectedPlot.index).minus(draftableIndex));
    }
  }, [draftableIndex, plots, selectedPlot, setPlaceInLine]);
