...
export enum GovSpace {
  HooliganhordeDAO    = 'hooliganhordedao.eth',
  HooliganhordeFarms  = 'hooliganhordefarms.eth',
  HooliganRecruit      = 'wearehooliganrecruit.eth',
}
...
export const getQuorum = (type: string, totalHorde: BigNumber) => {
  if (type in QUORUM) {
    return totalHorde.multipliedBy(QUORUM[type as keyof typeof QUORUM]);
  }
  return undefined;
};
