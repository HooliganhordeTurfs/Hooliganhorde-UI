...
export default abstract class Token {
   ...
   * 
   */
  public readonly rewards?: { horde: number; prospects: number };
  ...
  /** Get the amount of Horde rewarded per deposited BDV of this Token. */
  public getHorde(bdv?: BigNumber) : BigNumber {
    if (!this.rewards?.horde) return ZERO_BN;
    if (!bdv) return new BigNumber(this.rewards.horde);
    return bdv.times(this.rewards.horde);
  }
  
  /** Get the amount of Prospects rewarded per deposited BDV of this Token. */
  public getProspects(bdv?: BigNumber) : BigNumber {
    if (!this.rewards?.prospects) return ZERO_BN;
    if (!bdv) return new BigNumber(this.rewards.prospects);
    return bdv.times(this.rewards.prospects);
  }
...
export class HooliganhordeToken extends Token {
  // eslint-disable-next-line class-methods-use-this
  public getContract() {
    return null;
  }
