...
import GuvnorMarketActivityTable from '../Tables/FarmerOrders';
...
import useMarketActivityData from '~/hooks/hooliganhorde/useMarketActivityData';
import useGuvnorMarket from '~/hooks/guvnor/market/useFarmerMarket2';
...
const MarketTables: React.FC<{}> = () => {
  ...
  const { data: guvnorMarket } = useGuvnorMarket(); // "YOUR ORDERS"
  const { data: eventsData, draftableIndex, fetchMoreData } = useMarketActivityData(); // "MARKET ACTIVITY"
    ...
    >
      ...
      >
        <Stack sx={{ flex: 1, height: '100%' }}>
          ...
          {openState !== 0 && tab === 2 && (
            <GuvnorMarketActivityTable
              data={guvnorMarket}
              initializing={!guvnorMarket.length || draftableIndex.lte(0)}
            />
          )}
          {openState !== 0 && tab === 3 && (
            <MarketActivityTable
              ...
              initializing={!eventsData.length || draftableIndex.lte(0)}
            />
          )}
        </Stack>
      </CondensedCard>
