...
export function useFetchGuvnorAllowances() {
  ...
  const fetch = useCallback((
    ...
    if (_contract && account) {
      console.debug(`[guvnor/allowances/useFetchAllowances] FETCH account = ${account} contract = ${_contract} token(s) = ${_tokens.toString()}`);
      return Promise.all(
        ...
        console.debug(`[guvnor/allowances/useFetchAllowances] RESULT: ${_allowances.length} allowances`, _allowances);
  ...
  const clear = useCallback(() => {
    console.debug('[guvnor/allowances/useFetchAllowances] CLEAR');
