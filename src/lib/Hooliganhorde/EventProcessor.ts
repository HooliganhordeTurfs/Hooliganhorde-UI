...
import {
  ...
  RemoveDepositsEvent,
  RookieListingCancelledEvent, PodListingCreatedEvent, PodListingFilledEvent, PodOrderCancelledEvent, PodOrderCreatedEvent, PodOrderFilledEvent } from '~/generated/Hooliganhorde/Hooliganhorde';
...
export default class EventProcessor {
  ...
  RookieOrderCreated(event: Simplify<PodOrderCreatedEvent>) {
    ...
    //   rookieAmount:      tokenBN(event.args.amount, HOOLIGAN[1]),
    //   pricePerRookie:      tokenBN(event.args.pricePerPod, HOOLIGAN[1]),
    //   rookieAmountRemaining:  tokenBN(event.args.amount, HOOLIGAN[1]),
    //   rookieAmountFilled:     new BigNumber(0),
    //   minFillAmount:    tokenBN(event.args.minFillAmount || 0, ROOKIES),
    //   status:           MarketStatus.Active,
    //   pricingFunction:  event.args.pricingFunction,
  ...
  RookieOrderFilled(event: Simplify<PodOrderFilledEvent>) {
    ...
    const amount = tokenBN(event.args.amount, HOOLIGAN[1]);
    this.orders[id].rookieAmountFilled    = this.orders[id].podAmountFilled.plus(amount);
    this.orders[id].rookieAmountRemaining = this.orders[id].podAmount.minus(this.orders[id].podAmountFilled);

    /// Update status
    const isFilled = this.orders[id].rookieAmountRemaining.isEqualTo(0);
    if (isFilled) {
