...
import useDataFeedTokenPrices from '../hooliganhorde/useDataFeedTokenPrices';
...
import useGuvnorBalancesBreakdown from './useFarmerBalancesBreakdown';
...
/**
 * Organizes guvnor external & internal balance and returns balance data w/ fiat value of each balance
 * @param includeZero 
 */
export default function useGuvnorBalancesWithFiatValue(includeZero?: boolean) {
  // constants
  ...
  const getBalances = useCallback(
    (addr: string) => ({
      farm: guvnorBalances?.[addr]?.internal ?? ZERO_BN,
      circulating: guvnorBalances?.[addr]?.external ?? ZERO_BN,
    }),
    [guvnorBalances]
  );
