...
/**
 * Standard Hooligan price display: truncate with ROUND_FLOOR.
 * 
 * 0.99995 => "0.9999"
 * 1.00006 => "1.0000"
 * 
 * @param price Hooligan price
 * @param decimals number of decimals to display
 * @returns string; truncated Hooligan price
 */
export function displayHooliganPrice(
  price: BigNumber,
  decimals: number = 4,
) {
...
export function displayHorde(
  horde: BigNumber,
  decimals : number = HORDE.displayDecimals,
) {
  return horde.lt(0.0001) ? '0' : displayFullBN(horde, decimals);
}
