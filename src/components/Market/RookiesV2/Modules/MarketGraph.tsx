...
const axisColor      = HooliganhordePalette.lightGrey;
const tickLabelColor = HooliganhordePalette.lightGrey;
...
const TooltipCard : FC<CardProps> = ({ children, sx, ...props }) => (
  <Card sx={{ backgroundColor: HooliganhordePalette.lightestBlue, px: 0.5, py: 0.5, ...sx }} {...props}>
    {children}
  </Card>
);
...
const Graph: FC<GraphProps> = ({
  ...
  const orderPositions : CirclePosition[] = useMemo(() => 
    orders.map((order) => ({
      ...
      // y position is price per rookie
      y: yScale(order.pricePerRookie.toNumber()),
      // radius is plot size
      radius: 4,
    })),
  ...
  const listingPositions : CirclePosition[] = useMemo(() => 
    listings.map((listing) => ({
      ...
      // y position is price per rookie
      y: yScale(listing.pricePerRookie.toNumber()),
      // radius is plot size
  ...
  const orderCircles = useMemo(() => orderPositions.map((coordinate, i) => {
    ...
    return (
      <Circle
        ...
        fill={HooliganhordePalette.logoGreen}
        stroke={active ? HooliganhordePalette.mediumGreen : '#fff'}
  ...
  const listingCircles = useMemo(() => listingPositions.map((coordinate, i) => {
    ...
    return (
      <Circle
        ...
        fill={HooliganhordePalette.mediumRed}
        stroke={active ? HooliganhordePalette.trueRed : '#fff'}
        strokeWidth={active ? 2 : 1}
        cursor="pointer"
      ...
      ? (
        <g>
          <Line
            ...
            stroke={HooliganhordePalette.lightGrey}
            strokeWidth={1}
            pointerEvents="none"
          />
          <Line
            ...
            pointerEvents="none"
          />
          <Text fill={HooliganhordePalette.textBlue} x={cursorPoint.coordinate.x + 10} y={innerHeight - axis.xHeight} fontSize={14}>
            {displayBN(listings[cursorPoint.index].placeInLine)}
          </Text>
          <Text fill={HooliganhordePalette.textBlue} x={axis.yWidth + 10} y={cursorPoint.coordinate.y - 5} fontSize={14}>
            {listings[cursorPoint.index].pricePerRookie.toFixed(4)}
          </Text>
        </g>
      )
      : (
        <g>
          ...
          <Text fill={HooliganhordePalette.textBlue} x={axis.yWidth + 10} y={cursorPoint.coordinate.y - 5} fontSize={14}>
            {orders[cursorPoint.index].pricePerRookie.toFixed(4)}
          </Text>
        </g>
      )
    : null;
  ...
  const handleClick = useCallback(() => { 
    ...
    // If we're hovering a point, select it
      ...
      if (hoveredPoint.type === 'listing') {
        // handleClickFill(RookieOrderAction.BUY, PodOrderType.FILL);
        navigate(`/market/buy/${listings[hoveredPoint.index].id}`);
      } else if (hoveredPoint.type === 'order') {
        // handleClickFill(RookieOrderAction.SELL, PodOrderType.FILL);
        ...
        <Box sx={{ position: 'relative' }}>
          ...
          >
            ...
            <PatternLines
              ...
              height={5}
              width={5}
              stroke={HooliganhordePalette.logoGreen}
              strokeWidth={1}
          ...
          {/**
            * Show a tooltip with the number of Rookies whenever a point is
                ...
                <TooltipCard>
                  <Row gap={0.5}>
                    ...
                    {cursorPoint.type === 'listing'
                      ...
                      : displayBN(orders[cursorPoint.index].rookieAmountRemaining)
                    } Rookies
                  </Row>
                </TooltipCard>
              </Tooltip>
...
// const handleClickFill = useCallback((action: RookieOrderAction, type: PodOrderType) => {
...
// if (orderAction !== RookieOrderAction.BUY) {
//   setOrderAction(RookieOrderAction.BUY);
// }
// if (orderType !== RookieOrderType.ORDER) {
//   setOrderType(RookieOrderType.ORDER);
// }

// const [orderAction, setOrderAction] = useAtom(rookiesOrderActionTypeAtom);
// const [orderType, setOrderType] = useAtom(rookiesOrderTypeAtom);
