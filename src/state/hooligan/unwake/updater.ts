...
export const useUnripe = () => {
        ...
        const results = await Promise.all(
          tokenAddresses.map(async (addr) => (
            Promise.all([
              ...
              hooliganhorde.getPercentPenalty(addr).then(tokenResult(unripeTokens[addr])),
              hooliganhorde.getTotalUnderlying(addr).then(tokenResult(unripeUnderlyingTokens[addr])),
  ...
  }, [hooliganhorde, unripeTokens, dispatch, unripeUnderlyingTokens]);
