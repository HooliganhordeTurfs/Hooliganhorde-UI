...
import { GuvnorFirmRewardsQuery, SeasonalPriceQuery } from '~/generated/graphql';
import { secondsToDate, HORDE_PER_PROSPECT_PER_SEASON, toTokenUnitsBN } from '~/util';
...
/**
 * Interpolate a Guvnor's horde in a Season using past snapshots.
 * This calculates the amount of Grown Horde a Guvnor gains each season using their Prospects.
 */
export const interpolateGuvnorHorde = (
  ...
  for (let s = minSeason; s <= maxSeason; s += 1) {
    if (s === nextSeason) {
      ...
      currHorde = currHorde.plus(currProspects.multipliedBy(HORDE_PER_PROSPECT_PER_SEASON)); // Each Prospect grows 1/10,000 Horde per Season
  ...
  return [
    addBufferSeasons(horde, bufferSeasons, false),
    addBufferSeasons(prospects, bufferSeasons, false)
  ] as const;
};
 ...
 * and   (b) seasonal Hooligan price data.
 */
export const interpolateGuvnorDepositedValue = (
  snapshots: SnapshotHooliganhorde[], // oldest season first
  ...
  for (let s = minSeason; s <= maxSeason; s += 1) {
    ...
    if (s === nextSnapshotSeason) {
      ...
      for (j; snapshots[j]?.season === nextSnapshotSeason; j += 1) {
        const thisSnapshotBDV = toTokenUnitsBN(snapshots[j].hourlyDepositedBDV, HOOLIGAN[1].decimals);
