...
import { HOOLIGAN_CRV3_LP } from '~/constants/tokens';
...
> = ({
  ...
  const HooliganCrv3 = useChainConstant(HOOLIGAN_CRV3_LP);
                ...
                <AccordionDetails>
                  <TxnPreview
                    actions={[
                      ...
                      token === HooliganCrv3 && values.tokenOut !== token ? {
...
const Claim : FC<{
  ...
  const claimableBalance = firmBalance?.claimable.amount;
  ...
  const onSubmit = useCallback(async (values: ClaimFormValues, formActions: FormikHelpers<ClaimFormValues>) => {
    ...
    try {
      middleware.before();
      const crates = firmBalance?.claimable?.crates;
      ...
      txToast = new TransactionToast({
        loading: `Claiming ${displayTokenAmount(claimableBalance, token)} from the Firm...`,
      ...
      if (crates.length > 1) {
        ...
        data.push(
          hooliganhorde.interface.encodeFunctionData('claimWithdrawals', [
            token.address,
      ...
      // Claim a single withdrawal of `token` in one call. Gas efficient.
        ...
        data.push(
          hooliganhorde.interface.encodeFunctionData('claimWithdrawal', [
            token.address,
      ...
      const txn = await hooliganhorde.farm(data, {});
      txToast.confirming(txn);

      const receipt = await txn.wait();
      await Promise.all([refetchGuvnorFirm(), refetchFarmerBalances()]);
