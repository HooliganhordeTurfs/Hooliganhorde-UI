...
import hooliganhordeLogo from '~/img/tokens/hooligan-logo-circled.svg';
import ROUTES from './routes';
import MenuItemMobile from './MenuItemMobile';
import DropdownIcon from '~/components/Common/DropdownIcon';
import useToggle from '~/hooks/display/useToggle';
import useChainConstant from '~/hooks/chain/useChainConstant';
import { HOOLIGANHORDE_ADDRESSES, CHAIN_INFO } from '~/constants';
...
const NavDrawer: FC<{
  ...
  hideDrawer
    ...
    const hooliganhordeAddress = useChainConstant(HOOLIGANHORDE_ADDRESSES);
    ...
    return (
      ...
      >
        <Box position="fixed" sx={{ backgroundColor: 'background.paper', width: '100%', height: '100%', top: 0, overflowY: 'scroll' }}>
          {/* Hooliganhorde Logo & Close Button */}
          <Row alignItems="center" justifyContent="space-between" sx={{ p: 1.5 }}>
            <Box>
              <Link href="/" display="flex" alignItems="center">
                <img src={hooliganhordeLogo} alt="" width={IconSize.large} />
              </Link>
            </Box>
            ...
            <Box key="more" sx={{ borderBottom: 2, borderColor: 'divider' }}>
              ...
              <Stack display={openMore ? 'block' : 'none'}>
                ...
                <Box sx={{ px: 1, py: 0.6 }}>
                  <Button
                    fullWidth
                    href={`${chainInfo.explorer}/address/${hooliganhordeAddress}`}
                    target="_blank"
                    rel="noreferrer"
                    variant="contained"
                    color="primary"
                    ...
                    <Row alignItems="center" spacing={1}>
                      <ListItemText>
                        <Typography variant="h4">
                          Contract: {hooliganhordeAddress.slice(0, 6)}...
                        </Typography>
                      </ListItemText>
