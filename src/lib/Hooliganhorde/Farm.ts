...
import { Hooliganhorde__factory, Curve3Pool__factory, CurveTriCrypto2Pool__factory, CurveRegistry__factory, CurveMetaFactory__factory, CurveCryptoFactory__factory, CurveMetaPool__factory, CurvePlainPool__factory, CurveZap__factory } from '~/generated';
import { getChainConstant } from '~/util/Chain';
import { HOOLIGANHORDE_ADDRESSES, HOOLIGAN_CRV3_ADDRESSES, CRYPTO_FACTORY_ADDRESSES, CURVE_ZAP_ADDRESSES, META_FACTORY_ADDRESSES, POOL3_ADDRESSES, POOL_REGISTRY_ADDRESSES, TRICRYPTO2_ADDRESSES } from '~/constants';
import { HOOLIGAN, USDT, WETH } from '~/constants/tokens';
...
/**
 * Estimation Design Notes (Firm Chad)
 * -----------------------------------
 * 
 * Forward calculation:
 ...
 * 1567.31 USDT     1464.77 HOOLIGAN      get_dy_underlying     i=3, j=0     
 * -------------------------------------------------------------------------
 * amountOut = 1464.77 HOOLIGAN
 * 
 * Backward calcuation:
 ...
 *  - I want to buy Rookies from a Pod Listing. It's going to cost me 1,000 Hooligans but I only have ETH.
 *    How much ETH should I input to buy 1,000 Hooligans?
 *  - I want to Sow; there is 100 Rage available (so I can only Sow 100 Hooligans). How much ETH should I swap
 *    to HOOLIGAN to Sow all 100 Rage?
 * 
 * ? ETH <- 1000 HOOLIGAN
 * amountInStep  |  amountOutStep  |  lookup function    |  params
 * -------------------------------------------------------------------------
 * 1000 HOOLIGAN        1068.44 USDT      get_dy_underlying     i=0, j=3
...
const getContracts = (provider: ethers.providers.BaseProvider) => {
  ...
  const HOOLIGANHORDE       = getChainConstant(HOOLIGANHORDE_ADDRESSES, chainId);
  const POOL3           = getChainConstant(POOL3_ADDRESSES, chainId);
  const TRICRYPTO2      = getChainConstant(TRICRYPTO2_ADDRESSES, chainId);
  const HOOLIGANCRV3        = getChainConstant(HOOLIGAN_CRV3_ADDRESSES, chainId);
  ...
  const hooliganCrv3        = CurveMetaPool__factory.connect(HOOLIGANCRV3, provider);
  ...
  return {
    hooliganhorde: Hooliganhorde__factory.connect(HOOLIGANHORDE, provider),
    curve: {
      // Pools
      pools: {
        pool3,
...
export default class Farm {
  ...
  buyHooligans = (
    ...
    // USDT -> HOOLIGAN via hooligan3crv exchange_underlying
    this.exchangeUnderlying(
      this.contracts.curve.pools.hooliganCrv3.address,
      getChainConstant(USDT, this.provider.network.chainId).address,
      getChainConstant(HOOLIGAN, this.provider.network.chainId).address,
    ),
  ]

  pair = {
    WETH_HOOLIGAN: (
      _tokenIn : 'WETH' | 'HOOLIGAN',
      _fromMode? : FarmFromMode,
      _toMode?: FarmToMode,
    ) => {
      // default: WETH -> HOOLIGAN; flip if input is HOOLIGAN
      ...
      const Hooligan = getChainConstant(HOOLIGAN, this.provider.network.chainId).address;
  ...
  wrapEth = (
    ...
    return {
      ...
      encode: (_: ethers.BigNumber) => (
        this.contracts.hooliganhorde.interface.encodeFunctionData('wrapEth', [
          _amountInStep,        // ignore minAmountOut since there is no slippage
          _toMode,              //
        ])
      ),
      decode: (data: string) => this.contracts.hooliganhorde.interface.decodeFunctionData('wrapEth', data),
    };
  };

  unwrapEth = (
    /// Default to EXTERNAL because Hooliganhorde can't store INTERNAL ETH.
    ...
    return {
      ...
      amountOut: _amountInStep, // amountInStep should be an amount of ETH.
      encode: (_: ethers.BigNumber) => (
        this.contracts.hooliganhorde.interface.encodeFunctionData('unwrapEth', [
          _amountInStep,        // ignore minAmountOut since there is no slippage
          _fromMode,            //
        ])
      ),
      decode: (data: string) => this.contracts.hooliganhorde.interface.decodeFunctionData('unwrapEth', data),
    };
  };
  ...
  transferToken = (
    ...
    return {
      ...
      encode: (_: ethers.BigNumber) => (
        this.contracts.hooliganhorde.interface.encodeFunctionData('transferToken', [
          _tokenIn,      //
          _recipient,    //
          _amountInStep, // ignore minAmountOut since there is no slippage
          _fromMode,     //
      ...
      decode: (data: string) => this.contracts.hooliganhorde.interface.decodeFunctionData('transferToken', data),
    };
  };
  ...
  exchange = (
    ...
    return {
      ...
      encode: (minAmountOut: ethers.BigNumber) => (
        this.contracts.hooliganhorde.interface.encodeFunctionData('exchange', [
          _pool,
          _registry,
          tokenIn,
          tokenOut,
      ...
      decode: (data: string) => this.contracts.hooliganhorde.interface.decodeFunctionData('exchange', data),
      data: {
        pool: _pool,
        registry: _registry,
        tokenIn,
  ...
  exchangeUnderlying = (
    ...
    const amountOut = await CurveMetaPool__factory.connect(_pool, this.provider).callStatic['get_dy_underlying(int128,int128,uint256)'](
      i, // i = USDT = coins[3] ([0=HOOLIGAN, 1=CRV3] => [0=HOOLIGAN, 1=DAI, 2=USDC, 3=USDT])
      j, // j = HOOLIGAN = coins[0]
      _amountInStep,
      { gasLimit: 10000000 }
    );
    ...
    return {
      ...
      amountOut,
      encode: (minAmountOut: ethers.BigNumber) => (
        this.contracts.hooliganhorde.interface.encodeFunctionData('exchangeUnderlying', [
          _pool,
          tokenIn,
          tokenOut,
          _amountInStep,
      ...
      decode: (data: string) => this.contracts.hooliganhorde.interface.decodeFunctionData('exchangeUnderlying', data),
      data: {
        pool: _pool,
        tokenIn,
        tokenOut,
  ...
  addLiquidity(
    ...
    return async (_amountInStep: ethers.BigNumber) => {
      ...
      return {
        ...
        amountOut,
        encode: (minAmountOut: ethers.BigNumber) => (
          this.contracts.hooliganhorde.interface.encodeFunctionData('addLiquidity', [
            _pool,
            _registry,
        ...
        decode: (data: string) => this.contracts.hooliganhorde.interface.decodeFunctionData('addLiquidity', data),
        data: {
          pool: _pool,
          registry: _registry,
          fromMode: _fromMode,
  ...
  removeLiquidityOneToken(
    ...
    return async (_amountInStep: ethers.BigNumber) => {
      ...
      return {
        ...
        amountOut,
        encode: (minAmountOut: ethers.BigNumber) => (
          this.contracts.hooliganhorde.interface.encodeFunctionData('removeLiquidityOneToken', [
            _pool,
            _registry,
            _tokenOut,
            _amountInStep,
        ...
        decode: (data: string) => this.contracts.hooliganhorde.interface.decodeFunctionData('removeLiquidityOneToken', data),
        data: {}
      };
    };
  }
