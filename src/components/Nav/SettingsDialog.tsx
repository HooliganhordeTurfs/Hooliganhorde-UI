...
import { setNextSunrise, setRemainingUntilSunrise } from '~/state/hooliganhorde/sun/actions';
...
const SettingsDialog : FC<{ open: boolean; onClose?: () => void; }> = ({ open, onClose }) => {
  ...
  const exportDepositsCSV = useCallback(() => {
    const rows = Object.keys(firmBalances).reduce((prev, curr) => {
      prev.push(
        ...firmBalances[curr].deposited.crates.map((crate) => ([
          curr,
          crate.amount,
          crate.bdv,
          crate.season,
      ...
      return prev;
    }, [['Token', 'Amount', 'BDV', 'Season', 'Horde', 'Prospects']] as any[]);
    window.open(
      encodeURI(
