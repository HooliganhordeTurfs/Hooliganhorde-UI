...
import { EventCacheName, GuvnorEvents } from '~/state/guvnor/events2';

/**
 * Return the key at which Guvnor events should be held in localStorage.
 * @param chainId
 * @param account 
 * @param cacheId 
 * @returns string
...
export const loadState = () => {
  try {
    const serializedState = localStorage.getItem('hooliganhorde.s');
...
export const saveState = (state: any) => {
  if (state.app.settings) {
    try {
      ...
      localStorage.setItem('hooliganhorde.s', serializedState);
    } catch (err) {
      // pass
 ...
 * @unused
 */
export const rehydrateEvents2 = (events2: GuvnorEvents | undefined) => {
  try {
    if (!events2) return;
    const cache = { ...events2 };
