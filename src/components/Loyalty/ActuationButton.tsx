...
import sunIcon from '~/img/hooliganhorde/sun/sun-icon.svg';
...
const SunriseButton : FC<{}> = () => {
  ...
  const awaiting = useSelector<AppState, AppState['_hooliganhorde']['sun']['sunrise']['awaiting']>((state) => state._hooliganhorde.sun.sunrise.awaiting);
                  ...
                  <Stack gap={2}>
                    <Stack justifyContent="center" gap={2} py={2}>
                      ...
                      <Stack gap={1}>
                        ...
                        <Row justifyContent="center">
                          <Typography variant="body1">Reward for calling <Box display="inline" sx={{ backgroundColor: HooliganhordePalette.lightYellow, borderRadius: 0.4, px: 0.4 }}><strong><Link color="text.primary" underline="none" href="https://docs.hooligan.money/almanac/protocol/glossary#sunrise" target="_blank" rel="noreferrer">sunrise()</Link></strong></Box>: <TokenIcon token={HOOLIGAN[1]} />&nbsp;{displayBN(reward)}</Typography>
                        </Row>
                      </Stack>
                    </Stack>
                    <Divider />
                    <Typography sx={{ mx: 0 }} textAlign="center" variant="body1" color={HooliganhordePalette.washedRed}>Calling this function from the app is strongly discouraged because there is a high likelihood that your transaction will get front-run by bots.</Typography>
                    <LoadingButton
                      type="submit"
                      variant="contained"
                      onClick={onSubmit}
