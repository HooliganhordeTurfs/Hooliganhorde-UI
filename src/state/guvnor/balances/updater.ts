...
export const useFetchGuvnorBalances = () => {
  /// State
    ...
    try {
      if (account && erc20TokenMap) {
        ...
        const promises = Promise.all([
          ...
          hooliganhorde.getAllBalances(account, erc20Addresses)
            .then((result) => {
              console.debug('[guvnor/balances/updater]: getAllBalances = ', result);
              return result;
            })
        ...
        //   // from the standard getAllBalances call.
        //   multiCall.getEthBalance(account),
        //   wrap(hooliganhordeReplanted).getAllBalances(account, erc20Addresses)
        // ];

        console.debug(`[guvnor/updater/useFetchBalances] FETCH: balances (account = ${account})`);
        const balances = await promises;
        console.debug('[guvnor/updater/useFetchBalances] RESULT: ', balances);

        dispatch(updateBalances(balances));
        return promises;
      }
    } catch (e) {
      console.debug('[guvnor/updater/useFetchBalances] FAILED', e);
      console.error(e);
    }
  }, [
    dispatch,
...
const GuvnorBalancesUpdater = () => {
  const [fetch, clear] = useFetchGuvnorBalances();
  const account = useAccount();
...
export default GuvnorBalancesUpdater;
